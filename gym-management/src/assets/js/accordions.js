...
(function (factory) {
	...
	if (!$("<a>").outerWidth(1).jquery) {
		$.each(["Width", "Height"], function (i, name) {
			...
			function reduce(elem, size, border, margin) {
				$.each(side, function () {
					size -=
						parseFloat($.css(elem, "padding" + this)) || 0;
					if (border) {
						size -=
							parseFloat(
	...
	var accordion = $.widget("ui.accordion", {
		...
		_animate: function (toShow, toHide, data) {
			...
			duration =
				duration || options.duration || animate.duration;
	...
	var menu = $.widget("ui.menu", {
		...
		_scrollIntoView: function (item) {
			...
			if (this._hasScroll()) {
				...
				paddingTop =
					parseFloat(
						$.css(this.activeMenu[0], "paddingTop")
					) || 0;
				offset =
					item.offset().top -
	...
	$.widget("ui.button", {
		...
		_determineButtonType: function () {
			...
			if (
				...
				if (!this.buttonElement.length) {
					...
					this.buttonElement =
						ancestor.filter(labelSelector);
					if (!this.buttonElement.length) {
						this.buttonElement =
							ancestor.find(labelSelector);
					}
				}
		...
		_destroy: function () {
			...
			this.buttonElement
				.removeClass(
					baseClasses + " ui-state-active " + typeClasses
				)
				.removeAttr("role")
	...
	$.widget("ui.buttonset", {
		...
		refresh: function () {
			...
			this.buttons = allButtons
				...
				.filter(":first")
				.addClass(
					rtl ? "ui-corner-right" : "ui-corner-left"
				)
				.end()
				.filter(":last")
				.addClass(
					rtl ? "ui-corner-left" : "ui-corner-right"
				)
				.end()
				.end();
		},
   ...
   allowing multiple different settings on the same page. */
	...
	$.extend(Datepicker.prototype, {
		...
		_checkOffset: function (inst, offset, isFixed) {
			var dpWidth = inst.dpDiv.outerWidth(),
				...
				inputWidth = inst.input
					? inst.input.outerWidth()
					: 0,
				inputHeight = inst.input
					? inst.input.outerHeight()
					: 0,
				viewWidth =
		...
		_selectDate: function (id, dateStr) {
			...
			dateStr =
				dateStr != null ? dateStr : this._formatDate(inst);
			if (inst.input) {
		...
		parseDate: function (format, value, settings) {
			...
			var iFormat,
				...
				checkLiteral = function () {
					if (
						value.charAt(iValue) !== format.charAt(iFormat)
					) {
						throw (
		...
		_daylightSavingAdjust: function (date) {
			...
			date.setHours(
				date.getHours() > 12 ? date.getHours() + 2 : 0
			);
			return date;
		},
		...
		_generateHTML: function (inst) {
			...
			if (maxDate) {
				...
				maxDraw =
					minDate && maxDraw < minDate ? minDate : maxDraw;
				while (
					this._daylightSavingAdjust(
		...
		_generateMonthYearHeader: function (
			...
			if (secondary || !changeMonth) {
				...
				inMinYear =
					minDate && minDate.getFullYear() === drawYear;
				inMaxYear =
					maxDate && maxDate.getFullYear() === drawYear;
				monthHtml +=
		...
		_adjustInstDate: function (inst, offset, period) {
			var year =
					inst.drawYear + (period === "Y" ? offset : 0),
				month =
					inst.drawMonth + (period === "M" ? offset : 0),
				day =
					Math.min(
						inst.selectedDay,
		...
		_restrictMinMax: function (inst, date) {
			var minDate = this._getMinMaxDate(inst, "min"),
				maxDate = this._getMinMaxDate(inst, "max"),
				newDate =
					minDate && date < minDate ? minDate : date;
   ...
   @return  jQuery object */
	$.fn.datepicker = function (options) {
		...
		var otherArgs = Array.prototype.slice.call(
			arguments,
			1
		);
		if (
	...
	$.widget("ui.draggable", $.ui.mouse, {
		...
		_mouseCapture: function (event) {
			...
			if (
				this.helper ||
				o.disabled ||
				$(event.target).closest(".ui-resizable-handle")
					.length > 0
			) {
				return false;
			}
		...
		_cacheMargins: function () {
			this.margins = {
				...
				top:
					parseInt(this.element.css("marginTop"), 10) || 0,
				right:
	...
	$.ui.plugin.add("draggable", "stack", {
		start: function (event, ui, instance) {
			var min,
				o = instance.options,
				group = $.makeArray($(o.stack)).sort(function (
					a,
					b
				) {
					return (
		...
		_proportionallyResize: function () {
			...
			for (
				...
				prel.css({
					...
					width:
						element.width() - this.outerDimensions.width ||
						0,
				});
			}
		},
	...
	$.ui.plugin.add("resizable", "containment", {
		...
		resize: function (event) {
			...
			if (cp.top < (that._helper ? co.top : 0)) {
				...
				if (pRatio) {
					that.size.width =
						that.size.height * that.aspectRatio;
					continueResize = false;
				}
			...
			if (isParent && isOffsetRelative) {
				...
				that.offset.top =
					that.parentData.top + that.position.top;
			} else {
	...
	$.ui.plugin.add("resizable", "grid", {
		resize: function () {
			var outerDimensions,
				...
				gridY = grid[1] || 1,
				ox =
					Math.round((cs.width - os.width) / gridX) * gridX,
				oy =
					Math.round((cs.height - os.height) / gridY) *
					gridY,
				newWidth = os.width + ox,
				newHeight = os.height + oy,
				isMaxWidth = o.maxWidth && o.maxWidth < newWidth,
				isMaxHeight =
					o.maxHeight && o.maxHeight < newHeight,
	...
	var dialog = $.widget("ui.dialog", {
		...
		_makeDraggable: function () {
			...
			this.uiDialog.draggable({
				...
				stop: function (event, ui) {
					var left =
							ui.offset.left - that.document.scrollLeft(),
				...
				.resizable({
					...
					stop: function (event, ui) {
						var offset = that.uiDialog.offset(),
							left =
								offset.left - that.document.scrollLeft(),
		...
		_size: function () {
			...
			maxContentHeight =
				typeof options.maxHeight === "number"
					? Math.max(
							0,
							options.maxHeight - nonContentHeight
					  )
					: "none";
		...
		_allowInteraction: function (event) {
			...
			return !!$(event.target).closest(".ui-datepicker")
				.length;
		},
...
*/
	...
	(function (jQuery, undefined) {
		...
		support.rgba =
			supportElem.style.backgroundColor.indexOf("rgba") >
			-1;
		...
		color.fn = jQuery.extend(color.prototype, {
			parse: function (red, green, blue, alpha) {
				...
				if (type === "string") {
					return this.parse(
						stringParse(red) || colors._default
					);
				}
	...
	var effectBlind = ($.effects.effect.blind = function (
		o,
		done
	) {
		// Create element
	...
	var effectBounce = ($.effects.effect.bounce = function (
		o,
		done
	) {
		var el = $(this),
		...
		for (i = 0; i < times; i++) {
			...
			el.animate(upAnim, speed, easing).animate(
				downAnim,
				speed,
				easing
			);
	...
	var effectClip = ($.effects.effect.clip = function (
		o,
		done
	) {
		// Create element
	...
	var effectDrop = ($.effects.effect.drop = function (
		o,
		done
	) {
		var el = $(this),
	...
	var effectFade = ($.effects.effect.fade = function (
		o,
		done
	) {
		var el = $(this),
	...
	var effectFold = ($.effects.effect.fold = function (
		o,
		done
	) {
		// Create element
	...
	var effectHighlight = ($.effects.effect.highlight =
		function (o, done) {
			var elem = $(this),
				props = [
					"backgroundImage",
	...
	var effectSize = ($.effects.effect.size = function (
		o,
		done
	) {
		// Create element
		...
		if (o.mode === "toggle" && mode === "show") {
			...
			el.from =
				o.from || (mode === "show" ? zero : original);
		...
		if (origin) {
			...
			el.from.left =
				...
				baseline.x;
			el.to.top =
				(original.outerHeight - el.to.outerHeight) *
				baseline.y;
			el.to.left =
				(original.outerWidth - el.to.outerWidth) *
				baseline.x;
		}
		...
		if (scale === "content" || scale === "both") {
			...
			el.find("*[width]").each(function () {
				...
				child.from = {
					...
					outerHeight:
						c_original.outerHeight * factor.from.y,
	...
	var effectScale = ($.effects.effect.scale = function (
		o,
		done
	) {
		// Create element
	...
	var effectPuff = ($.effects.effect.puff = function (
		o,
		done
	) {
		var elem = $(this),
	...
	var effectPulsate = ($.effects.effect.pulsate = function (
		o,
		done
	) {
		var elem = $(this),
	...
	var effectShake = ($.effects.effect.shake = function (
		o,
		done
	) {
		var el = $(this),
		...
		// Animation
		animation[ref] =
			(positiveMotion ? "-=" : "+=") + distance;
		animation1[ref] =
			(positiveMotion ? "+=" : "-=") + distance * 2;
		animation2[ref] =
			(positiveMotion ? "-=" : "+=") + distance * 2;
	...
	var effectSlide = ($.effects.effect.slide = function (
		o,
		done
	) {
		// Create element
	...
	var effectTransfer = ($.effects.effect.transfer =
		function (o, done) {
			var elem = $(this),
				target = $(o.to),
	...
	var slider = $.widget("ui.slider", $.ui.mouse, {
		...
		_createHandles: function () {
			...
			if (existingHandles.length > handleCount) {
				...
				existingHandles = existingHandles.slice(
					0,
					handleCount
				);
			}
		...
		_calculateNewMax: function () {
			var remainder =
				(this.options.max - this._valueMin()) %
				this.options.step;
	...
	var sortable = $.widget("ui.sortable", $.ui.mouse, {
		...
		_mouseCapture: function (event, overrideHandle) {
			...
			$(event.target)
				...
				.each(function () {
					if (
						$.data(this, that.widgetName + "-item") === that
					) {
						currentItem = $(this);
						return false;
					}
			...
			if (
				$.data(event.target, that.widgetName + "-item") ===
				that
			) {
		...
		_intersectsWith: function (item) {
			var x1 = this.positionAbs.left,
				...
				isOverElement =
					isOverElementHeight && isOverElementWidth;
		...
		_contactContainers: function (event) {
			...
			if (this.containers.length === 1) {
				if (
					!this.containers[innermostIndex].containerCache
						.over
				) {
		...
		_cacheMargins: function () {
			this.margins = {
				...
				top:
					parseInt(this.currentItem.css("marginTop"), 10) ||
					0,
			};
		},
		...
		_trigger: function () {
			if (
				$.Widget.prototype._trigger.apply(
					this,
					arguments
				) === false
			) {
	...
	var spinner = $.widget("ui.spinner", {
		...
		_draw: function () {
			...
			if (
				this.buttons.height() >
					Math.ceil(uiSpinner.height() * 0.5) &&
				uiSpinner.height() > 0
			) {
		...
		_spin: function (step, event) {
			...
			if (
				!this.spinning ||
				this._trigger("spin", event, { value: value }) !==
					false
			) {
				this._value(value);
				this.counter++;
		...
		_adjustValue: function (value) {
			...
			aboveMin =
				Math.round(aboveMin / options.step) * options.step;
		...
		_setupDisabled: function (disabled) {
			if ($.isArray(disabled)) {
				if (!disabled.length) {
					...
					disabled.length === this.anchors.length
				) {
					disabled = true;
				}
			}
