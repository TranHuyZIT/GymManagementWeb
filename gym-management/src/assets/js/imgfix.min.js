...
!(function (a) {
	function t(t) {
		...
		(null != s.data("fix-defaults-grayscale") ||
			...
			(c +=
				null != s.data("fix-defaults-blur")
					? "blur(" +
					  parseInt(s.data("fix-defaults-blur")) +
					  "px) "
					: "")),
			c &&
				(c =
					"filter: " + c + "; -webkit-filter: " + c + ";");
		var p = "";
		(null != s.data("fix-grayscale") ||
			...
			(p +=
				null != s.data("fix-brightness")
					? "brightness(" +
					  parseInt(s.data("fix-brightness")) +
					  "%) "
					: null != s.data("fix-defaults-brightness")
					? "brightness(" +
					  parseInt(s.data("fix-defaults-brightness")) +
	...
	function s(a, t) {
		"imgfix-in-up" != t && a.removeClass("imgfix-in-up"),
			"imgfix-in-right" != t &&
				a.removeClass("imgfix-in-right"),
			"imgfix-in-down" != t &&
				a.removeClass("imgfix-in-down"),
			"imgfix-in-left" != t &&
				a.removeClass("imgfix-in-left"),
			"imgfix-in-up-left" != t &&
				a.removeClass("imgfix-in-up-left"),
			"imgfix-in-up-right" != t &&
				a.removeClass("imgfix-in-up-right"),
			"imgfix-in-down-left" != t &&
				a.removeClass("imgfix-in-down-left"),
		...
		init: function (e, s) {
			...
			l.cover || (l.cover = {}),
				...
				(l = {
					...
					interval:
						parseInt(l.interval) >= 0 ? l.interval : 400,
					grayscale:
						parseInt(l.grayscale) >= 0 ? l.grayscale : null,
					sepia: parseInt(l.sepia) >= 0 ? l.sepia : null,
					hue: parseInt(l.hue) >= 0 ? l.hue : null,
					saturate:
						parseInt(l.saturate) >= 0 ? l.saturate : null,
					invert: parseInt(l.invert) >= 0 ? l.invert : null,
					brightness:
						parseInt(l.brightness) >= 0
							? l.brightness
							: null,
					contrast:
						parseInt(l.contrast) >= 0 ? l.contrast : null,
			...
			for (var n = 0; n < d.length; n++) {
				...
				var u = o.find(
					"#imgfix_wrapper_layer_" + e.id + "_" + n
				).length
					? 0
					: 1;
				if (s)
					...
					if (o.data("fix-protect-aspect-ratio"))
						if (o.data("fix-aspect-ratio"))
							o.height(
								o.width() * o.data("fix-aspect-ratio")
							);
						else {
							var x = o.height(),
								c = o.width(),
				...
				if (
					...
					0 == o.find("img").length
						...
						: a(o.find("img")[0]).addClass(
								"imgfix_src_img"
						  );
					for (
						var g = o.find("img"), v = 0;
						v < g.length;
						v++
					)
						a(g[v]).data(
